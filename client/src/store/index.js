...
export const GlobalStoreActionType = {
    ...
    GET_PUBLISHED_PLAYLISTS: "GET_PUBLISHED_PLAYLISTS"
}
...
function GlobalStoreContextProvider(props) {
    ...
    const storeReducer = (action) => {
        ...
        switch (type) {
            ...
            case GlobalStoreActionType.GET_PLAYLIST_FROM_USER: {
                return setStore({
                    currentModal: CurrentModal.NONE,
                    idNamePairs: store.idNamePairs,
                    currentList: store.currentList,
            ...
            case GlobalStoreActionType.GET_PUBLISHED_PLAYLISTS: {
                return setStore({
                    currentModal: CurrentModal.NONE,
                    idNamePairs: store.idNamePairs,
                    currentList: store.currentList,
    ...
    store.addNewSong = function() {
        ...
        this.addCreateSongTransaction(index, "Untitled", "Undefined", "dQw4w9WgXcQ");
    }
    ...
    store.addNewSong = () => {
        ...
        store.addCreateSongTransaction(
            playlistSize, "Untitled", "Undefined", "dQw4w9WgXcQ");
    }
    ...
    store.publish = function(id){
        async function getPlaylistAndConvertDate(id){
            let response = await api.getPlaylistById(id);
            if (response.data.success){
                let playlist = response.data.playlist;
                console.log(playlist);
                let currentTime = new Date();
                let dateOfPublish = currentTime.toLocaleDateString();
                console.log(dateOfPublish);
                playlist.publish = true; //Say the playlist is published
                playlist.publishDate = dateOfPublish; //Updated Playlist publish date
                console.log(playlist);
                ...
                async function updateLists(playlist){
                    ...
                    if (response.data.success){
                        // async function listPairs(playlist){
        ...
        getPlaylistAndConvertDate(id);
    }

    store.getUserPlaylists = function() {
        console.log("Inside getUserLists");
        let user = auth.user;
        console.log(user);
        async function getLists(user){
            let response = await api.getPlaylists(user);
            console.log(response);
            if (response.data.success){
                let playlist = response.data.playlist;
                console.log(playlist);
    ...
    store.renderNoPlaylists = function() {
        storeReducer({
            type: GlobalStoreActionType.GET_PLAYLIST_FROM_USER,
            payload: []
        })
    ...
    store.addComment = function (username, userComment, playlistID){
        console.log(userComment);
        async function comment(username, userComment, playlistID){
            let response = await api.addComment(username, userComment, playlistID);
            console.log(response);
            if (response.data.success){
                let playlist = response.data.playlist;
                console.log(playlist);
                ...
                console.log("API FAILED TO ADD A COMMENT");
            }
        }
        comment(username, userComment, playlistID);
    }

    store.filterPublishedPlaylists = function (){
        console.log("Inside filter playlist");
        async function filterPublishedPlaylists(){
            ...
            if (response.data.success){
                ...
                console.log("Failed getting published playlists");
            }
        }
